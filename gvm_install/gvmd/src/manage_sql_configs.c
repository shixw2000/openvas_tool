...
/**
 * @brief Validate an multi config list.
 *
 * @param[in]  list  Comma separated list of config.
 *
 * @return 0 success, 1 usage_type invalid, 2 multi_config_id invalid,
 *     3 name invalid,
 *     4 name already exists,
 *     99 permission denied, -1 error.
 */
static int validate_multi_config_param(const char* name, 
    const char* comment, const char* usage_type, 
    const char *multi_config_id) {
    const char* type = "config";
    gchar* quoted_name = NULL;
    gchar **split, **point; 

    if (NULL == usage_type || strcasecmp (usage_type, "scan") != 0) {
        return 1;
    }

    if (NULL == multi_config_id || strlen (multi_config_id) == 0) {
        return 2;
    }
    ...
    split = g_strsplit (multi_config_id, ",", 0); 
    if (split[0] == NULL) {
        g_strfreev (split);
        return 2;
    } 
    ...
    point = split;
    while (*point) {
        const char *resource_id = *point;
        config_t resource;

        if (find_resource_with_permission (type, resource_id, &resource, "get_configs", 0)
            || 0 == resource) {
            g_strfreev (split);
            return 2;
        }
...
/**
 * @brief Create a config from an existing multi-config.
 *
 * @param[in]  name        Name of new config.
 * @param[in]  comment     Comment on new config. 
 * @param[in]  usage_type  Optional new usage type for the new config.
 * @param[in]  multi_config_id   UUID of existing config.
 * @param[out] new_config  New config.
 *
 * @return 0 success, 1 usage_type invalid, 2 multi_config_id invalid,
 *     3 name invalid,
 ...
 *     99 permission denied, -1 error.
 */
int multi_copy_config (const char* name, 
    const char* comment, const char* usage_type, 
    const char *multi_config_id, config_t* new_config) {
    int ret;
    char* config_uuid = NULL;
    char* selector_uuid = NULL;
    const char *old_config_uuid = NULL;
    gchar **split, **point;
    gchar* quoted_name = NULL;
    gchar *quoted_comment = NULL;
    config_t new_id = 0;

    config_uuid = gvm_uuid_make ();
    selector_uuid = gvm_uuid_make ();
    if (NULL == selector_uuid || NULL == config_uuid) {
        return -1;
    } 

    sql_begin_immediate ();
    
    ret = validate_multi_config_param(name, comment, usage_type, multi_config_id);
    if (0 != ret) {
        sql_rollback();
        return ret;
    } 
    ...
    if (comment) {
        ...
        sql ("INSERT INTO configs (uuid, name, owner, nvt_selector,"
            " type, creation_time, modification_time, usage_type, predefined)"
            " VALUES ('%s', '%s',"
            " (SELECT id FROM users WHERE users.uuid = '%s'),"
            " '%s', '0', m_now (), m_now (), 'scan', '0');",
            config_uuid,
            quoted_name,
            current_credentials.uuid,
            selector_uuid);
    ...
    g_free(quoted_name);
    new_id = sql_last_insert_id (); 

    /* copy multi config nvts to this one */
    split = g_strsplit (multi_config_id, ",", 0); 
    point = split;
    while (*point) {
        old_config_uuid = *point; 
        ...
        sql ("INSERT INTO config_preferences"
            ...
            new_id, old_config_uuid);

        sql ("INSERT INTO nvt_selectors (name, exclude, type, family_or_nvt, family)"
            " SELECT '%s', exclude, type, family_or_nvt, family"
            " FROM nvt_selectors"
            " WHERE name = (select nvt_selector from configs where uuid='%s');",
            selector_uuid,
            old_config_uuid);
