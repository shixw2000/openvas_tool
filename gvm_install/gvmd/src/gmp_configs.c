...
create_config_run (gmp_parser_t *gmp_parser, GError **error)
{
  ...
  if (scanner && strlen (entity_text (scanner)))
    ...
    /* Check for creation from multi-copy. */ 
    entity_t multi_copy = entity_child (entity, "multi_copy");
    if (multi_copy && strlen (entity_text (multi_copy))) { 
        config_t new_config;

        switch (multi_copy_config(
            text_or_null (entity_child (entity, "name")),
            text_or_null (entity_child (entity, "comment")),
            text_or_null (entity_child (entity, "usage_type")),
            entity_text (multi_copy),
            &new_config))
        {
          case 0:
            {
              char *uuid = config_uuid (new_config);
              SENDF_TO_CLIENT_OR_FAIL (XML_OK_CREATED_ID ("create_config"), uuid);
          ...
          case 1:
            SEND_TO_CLIENT_OR_FAIL
             (XML_ERROR_SYNTAX ("create_config", "usage_type is invalid"));
            log_event_fail ("config", "Scan config", NULL, "created");
            break;
          case 2:
            SENDF_TO_CLIENT_OR_FAIL
             (XML_ERROR_SYNTAX ("create_config", "Config uuid is invalid"));
            
            log_event_fail ("config", "Config", NULL, "created");
            break;
          case 3:
            SENDF_TO_CLIENT_OR_FAIL
             (XML_ERROR_SYNTAX ("create_config", "Config name is invalid"));
            log_event_fail ("config", "Scan config", NULL, "created");
            break;
          case 4:
            SENDF_TO_CLIENT_OR_FAIL
             (XML_ERROR_SYNTAX ("create_config", "Config name exists already"));
            log_event_fail ("config", "Scan config", NULL, "created");
            break;
            
          case 99:
            SEND_TO_CLIENT_OR_FAIL
             (XML_ERROR_SYNTAX ("create_config", "Permission denied"));
            log_event_fail ("config", "Scan config", NULL, "created");
            break;
          case -1:
          default:
