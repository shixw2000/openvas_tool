                  ...
                  int *trust)
{
  ...
  char installer_file[] = "/opt/gvmd-installer-XXXXXX";
  char signature_file[] = "/opt/gvmd-signature-XXXXXX";
  GError *error = NULL;
  ...
  cmd[9] = NULL;
  g_debug ("%s: Spawning in /opt/: %s %s %s %s %s %s %s %s %s",
           __func__,
      ...
      while (*current_rf_dependency)
        {
          ...
          subreport_dir = g_strdup ("/opt/gvmd_XXXXXX");
